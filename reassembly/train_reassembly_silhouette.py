...
if ARG_save_mode == 'new':
    RUNS_FOLDER = 'results/runs_'+str(_num+1).zfill(4)
elif ARG_save_mode == 'override' and _num > 0:
    RUNS_FOLDER = 'results/runs_'+str(_num).zfill(4)
...
for i in range(hyperparameters.CAMERA_COUNT):
    ...
    __targetSilhouette__ = renderer.project_silhouette([target_mesh.mesh]).to(device)
    target_silhouettes.append(__targetSilhouette__)
...
for i in range(hyperparameters.CAMERA_COUNT):
    renderer = renderers[i]
    silhouette = renderer.project_silhouette([mesh.mesh for mesh in __sourceMeshFields__]) # project the source objects
...
losses = {  
            "silhouette"    : {"weight": hyperparameters.WEIGHT_SILHOUETTE, "values": []},
...
try:
    for __iteration__ in iterations:
        ...
        # compute the losses
        for renderer, __targetSilhouette__ in zip(renderers, target_silhouettes):
            loss["silhouette"] += Losses.get_silhouette_loss(renderer, __deformedMeshFields__, __targetSilhouette__)
            # loss["LoG"] += Losses.get_LoG_loss(renderer, __deformedMeshFields__, __targetSilhouette__)
        ...
        iterations.set_description("Silhouette: %.3f, Intersection: %.3f, Bounding: %.3f" % (losses["silhouette"]["values"][-1], losses["intersection"]["values"][-1], losses["bounding"]["values"][-1]))
        ...
        if hyperparameters.PROGRESSIVE_RESULTS:
            ...
            for i in range(hyperparameters.CAMERA_COUNT):
                renderer = renderers[i]
                silhouette = renderer.project_silhouette([mesh.mesh for mesh in __deformedMeshFields__])
        ...
        if (__iteration__+1) % hyperparameters.PLOT_PERIOD == 0:
            ...
            visualizer.save_obj([i.mesh for i in __deformedMeshFields__], name=RUNS_FOLDER+'/structures/structure_' + str(__iteration__+1))
    ...
    visualizer.visualize_curves([
        {"values" : losses["silhouette"]["values"], "name" : "silhouette"},
