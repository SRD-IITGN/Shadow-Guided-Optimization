...
for i in range(hyperparameters.CAMERA_COUNT):
        ...
        renderer = Rendering(camera_transform, 
                             ...
                             mode='texture',
                             device=device)
        renderers_texture.append(renderer)
        renderer = Rendering(camera_transform, 
                             camera_type=hyperparameters.CAMERA_TYPES[i], 
                             image_size=hyperparameters.IMAGE_SIZE, 
                             sigma=1e-4, 
                             ...
                             mode='silhouette',
                             device=device)
        renderers_silhouette.append(renderer)
...
for trg_ in hyperparameters.TARGET_OBJECTS:
    ...
    cols = torch.FloatTensor(1, 3).uniform_(0.2, 1).to(device)
...
for i in range(hyperparameters.CAMERA_COUNT):
    ...
    __targetTexture__ = renderers_texture[i].project_texture([target_mesh.mesh]).to(device)
    __targetSilhouette_= renderers_silhouette[i].project_silhouette([target_mesh.mesh]).to(device)
    target_textures.append(__targetTexture__)
    target_silhouettes.append(__targetSilhouette_)

visualizer.visualize_silhouette_multiple(target_textures, name=RUNS_FOLDER+'/metadata/target_textures')
...
source_silhouettes = []
for i in range(hyperparameters.CAMERA_COUNT):
    texture = renderers_texture[i].project_texture([mesh.mesh for mesh in __sourceMeshFields__]) # project the source objects
    source_textures.append(texture)
    silhouette = renderers_silhouette[i].project_silhouette([mesh.mesh for mesh in __sourceMeshFields__]) # project the source objects
...
visualizer.visualize_silhouette_multiple(source_textures, name=RUNS_FOLDER+'/metadata/source_textures') 
...
transformations= []
for i in range(hyperparameters.NUM_OBJECTS):
...
# setup the optimizer
# optimizer = torch.optim.SGD(transformations, lr=0.01, momentum=0.9) 
...
try:
    for __iteration__ in iterations:
        ...
        for instance in range(hyperparameters.NUM_OBJECTS):
        ...
        # compute the losses
        for renderer, __targetTexture__, __targetSilhouette__ in zip(renderers_texture, target_textures, target_silhouettes):
            loss["texture"] = Losses.get_silhouette_aware_texture_loss(renderer, __deformedMeshFields__, __targetSilhouette__, __targetTexture__)
        ...
        if hyperparameters.PROGRESSIVE_RESULTS:
            _textures = []
            for i in range(hyperparameters.CAMERA_COUNT):
                renderer = renderers_texture[i]
                silhouette = renderer.project_texture([mesh.mesh for mesh in __deformedMeshFields__])
                silhouette = silhouette.detach().cpu()
                _textures.append(silhouette)
            visualizer.visualize_silhouette_multiple(_textures, name=RUNS_FOLDER+'/silhouettes')
        ...
        if (__iteration__+1) % hyperparameters.PLOT_PERIOD == 0:
            ...
            for i in range(hyperparameters.CAMERA_COUNT):
                ...
                silhouette = silhouette.detach().cpu()
                _textures.append(silhouette)
                visualizer.visualize_silhouette_multiple(_textures, name=RUNS_FOLDER + '/silhouettes/image' + '_' + str(__iteration__+1).zfill(6))
